...
MPI ranks must know the destination before they can send their data, so if an early ranks needs to send data to a later rank it can either poll that rank's location in remote memory (an inadvisable choice unless you're trying to pick a fight with your sysadmin), or the late rank can notify the earlier process once it arrives.
...
If we simplify the problem and look at a 2 process allreduce, the minimum amout of work involbes recieving a message, performing a local reduction and sending a message.
