...
We also evaluated the effects of remapping allreduce on data-parallel DL training, but there was no observable impact.
We tested a series of models using Synthetic datasets across CPU and GPU resources on Beluga and Cedar, we also evaluated the same set of initial mappings, but there was no discernible difference in performance between remapped and non-remapped collectives.
...
So while microbenchmarks do show that a topology-aware remapping can improve allreduce performance, the benefits are not present in the message sizes used for data-parallel DL training with Horovod.
