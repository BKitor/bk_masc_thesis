...
We evaluated our new heuristics, as well as the existing heuristics, on CPU and GPU allocations accross two different clusters.
To ensure the robustness of our algorithm, we evaluate accross different inital mapping to stress test different starting conditions.
Furthurmore, to evaluate against more tradional topology mapping methods, we compare our heuristics against SCOTCH, a commonly used graph partitioning library previously applied to topology-aware works.
...
We implemented our method by modidfying OpenMPI v4.0.5 \cite{gabriel2004OpenMPI}.
...
Out of the box, OpenMPI's relies on a static tuning table to chose a collective algorithm based on the communicator size and message size, so we applied SCOTCH to whichever algorithm the tuning table selected.
...
It also has a fat-tree topology configured, but it is configured with 2:1 blocking and has adaptive routing.
...
One other key difference is the PCIe hierarchy connecting CPUs to GPUs.
On Beluga all the GPUs are connected to socket 0 through a PCI switch, while Cedar, on the other hand, has GPUs split between sockets 0 and 1 each connecting to the socket.

\subsection{Results}

\subsubsection{Performance}
\input{3_Chapters/4_Chapter_TopologyAwareness/Figures/log_plots/ar_beluga_128}
\input{3_Chapters/4_Chapter_TopologyAwareness/Figures/log_plots/bc_beluga_128}

\input{3_Chapters/4_Chapter_TopologyAwareness/Figures/log_plots/ar_cedar_32g_64c}
\input{3_Chapters/4_Chapter_TopologyAwareness/Figures/log_plots/bc_cedar_32g_64c}

We used OSU-Microbenchamrks v5.7 \cite{Bureddy2012OMB} to evaluate the performance of our proposed method. 
This tool loops over MPI\_Allreduce, measures the runtime of each function call, and outputs the average latency. 
It also provides the option to locate the data buffer in either CPU or GPU memory.

The GPU nodes were evaluated with four processes per node as that is the number of GPUs each node has, and CPU allocations were evaluated with 32 processes per node since it's the largest power of two that can fit on a node before oversubscription.
We were able to evaluate with large allocations on Beluga, reserving 128 nodes at a time for both CPU and GPU partitions, this gives us 512 GPU processes total and 4096 CPU processe total.
...
Cedar's alocations were a bit smaller with 32 GPU nodes (communicator size of 128) and 64 CPU nodes (communicator size of 2048).
The Allreduce resutls on Cedar are presented in figure \ref{fig:cedar-ar-32g64c}, and Broadcast results are in figure \ref{fig:cedar-bc-32g64c}.
While reuslts on Cedar are not identical to Beluga, due to differences in computer archiceture and experiment setup, the benifits of our proposed methods are consistant accross both clusters.

OpenMPI's collective algorihms tend to be structured so that processes exchange most of their data with nerby ranks, this design characteristic is most pronounced in algorithms like RSA, ring, and knomial.
This design decision tends to perform well on by-socket and by-core mappings, and this reflects in our results where default OpenMPI performs similarly to our remaped algorithms on by-core and by-socket initlal mappings.
But, the same algorithms tend to strugle with by-node inital mappings because each process's nearby ranks are off-node, which leads to a lot more data traversing the fabric.
The topology-awareness of our remaping method solves this issue, and releives network pressure by moving most of the communication back within the node, and tends to outperform the default by a large margin.
While there are scenarios where SCOTCH can improve performace, it often has a negative impact on performance.
So while it can efficiently create a mapping, our heuristics often achive beter resutls, and the initial conditions are often accounted for in algorithm design.

Topology-awareness also can also releive contention for resources like network cards or inter-socket links. 
This is best observed by comparing by-node GPU results to by-node CPU results.
...
While both allocations benifit from remapping, the results are more impactful for the CPU jobs as they can redirect much more of the resuorce demand.
While not explicitly demonstrated, the same idea can be seen by remapping a by-socket ring algorithm.
By defualt all messages would be sent accross sockets, forcing the UPI link to beceme a bottleneck, reordering the ranks so that only one set of processes exchange data accross sockets can provide up to 50\% performance improvemnt for large messages.

\subsubsection{Scalability}

\input{3_Chapters/4_Chapter_TopologyAwareness/Figures/log_plots/beluga_scale}

The remaping algorithm does retain its performance wins accross communicator sizes. 
...
Only a by-socket mapping is presented, but by-core and by-node have similar characteristics.
Some of the heuristics are still hampered by their underlying algorithm, recursive doubling being the worst offender, but the performance improvements scale well accross the board.

Focus on CPU bcast? OMPI seems to be scaling worse than remaped algs
