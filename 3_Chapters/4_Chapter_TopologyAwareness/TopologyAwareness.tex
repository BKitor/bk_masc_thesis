...
RSA is used for large message allreduce because it is bandiwth efficient.
...
To account for a potentialy lopsided tree, the heuristics traverses the potentialy smaller side first by mapping the larger child first, (lines 27-23).
...
While the structue of the tree is lopsided, in theory this provides the opertunity to overlap message transferts lower in the tree with transfers higher in the tree.
Due to the inductive definitions, high $k$ value trees will have a lot of leaf nodes, as each subtree will contain of $k-1$ subtrees with $h=1$.
...
data-transfer for bcast
Don't support \_ reductions

While any value of $k$ can be used, in OpenMPI $k$ is usualy set to 4.
